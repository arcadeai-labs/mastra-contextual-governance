...
- Always pass `--setup run`. The repo setup hook claims a block of ten ports,
  writes `CG_PORT_*` into an untracked root `.env.local` plus a per-service
  `.env.local` carrying each service's own `PORT`, and runs **both** installs
  (`apps/idp` is outside the workspace). You never assign ports; you only check
  the hook ran if a worker reports a collision.
...
2. `gh issue list --repo <repo> --state open --json number,title,body` and
   ...
   **The line is mandatory and its absence is not "none".** An issue with no
   `Blocked by` line is not dispatchable: you cannot tell an issue nobody
   blocked from an issue nobody finished writing. Do not guess and do not
...
## Orca CLI facts, measured on this project

These cost a round each to learn. They are not project-specific — they belong
upstream in the template — but until they are there, they live here.

- **`check --wait` is what blocks.** `--timeout-ms` on its own returns
  `No messages.` in 0.15s. Issue the wait as its own tracked background call,
  never appended to a compound command with `&`: an untracked waiter loses its
  notifications.
- **A run holds exactly one waiter.** A second `--wait` fails `waiter_exists`,
  and a plain `check` or `--ack` does **not** cancel a live one. After acking a
  heartbeat, do nothing — the existing wait still covers you. Only re-arm once
  the waiter's own process has exited.
- **`check --ack` takes the `deliveryId`** from the top of the response, not the
  `id` on the message. Acking a replayed question does not answer it.
- **`orchestration reply` takes `--id`**, not `--to-message`, and a reply alone
  does not reliably wake the worker — follow it with
  `orchestration send --to dispatch:<id>`.
- **Never put backticks in a `--body` argument.** The shell reads them as
  command substitution. Write the body to a file and pass `--body "$(cat file)"`.
- **Keep replies short and put the verdict in the first two lines.** A long
  reply arrived truncated once, and the verdict was in the missing third.
- **`gh pr merge --delete-branch` does not delete the remote ref on this repo —
  nine for nine.** Always follow with `git push origin --delete <branch>` and
  prove it with `git ls-remote`.
- **Never read another branch by checking its files into the main worktree.**
  Use `git grep <rev>` or `git show <rev>:<path>`.

## Every worker spec carries the external-state ban

Both role prompts point at `.orca/project.md`, but the ban is cheap to restate
and expensive to omit. Every `{{DELTA}}` and `{{EXTRA}}` you fill in must say
explicitly: do not deploy, provision, log in, authenticate, use or create
credentials, or alter external configuration; and do not point anything at the
live hosts named under "Non-negotiables" in `.orca/project.md`. GitHub
operations scoped to the worker's own issue and PR remain permitted.
