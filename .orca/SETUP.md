...
Expected in the root file: `CG_PORT_BASE`, `CG_PORT_WEB`, `CG_PORT_HOOKS`,
`CG_PORT_LOAN_APP`, `CG_PORT_IDP` and the cross-service host strings — and **no
bare `PORT`**, which anything run from the repo root would otherwise inherit.
Then check the per-service files, which are where `PORT` actually lives:

```sh
cat apps/hooks/.env.local     # PORT=<base+1>
```

All four services read the same `PORT` variable, so one shared file cannot
carry four values. The hook is idempotent and keeps the block this worktree
already holds. It also runs **both** installs: `bun install` and
`bun install --cwd apps/idp`, because `apps/idp` is not a workspace member.
